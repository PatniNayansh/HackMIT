...
def test_tier_steps_differ_in_fill_not_only_in_colour():
    ...
    assert 'h("button", {\n    class: `tier-chip' in (FRONTEND_DIR / "js" / "run-common.js").read_text()  # label text is the button's content
    assert "}, t.label)" in (FRONTEND_DIR / "js" / "run-common.js").read_text()
...
def test_expert_series_is_told_apart_from_the_reference_by_stroke_width_and_dash():
    src = (FRONTEND_DIR / "js" / "charts.js").read_text()
...
def test_every_slide_image_in_the_ui_goes_through_the_slide_card():
    ...
    assert 'class: `slide-card' in (FRONTEND_DIR / "js" / "dom.js").read_text()
    assert 'class: "thumb slide-card"' in (FRONTEND_DIR / "js" / "views-overview.js").read_text()
...
def test_the_toggle_persists_follows_the_system_and_survives_a_throwing_localstorage():
    p = subprocess.run(["node", str(FRONTEND_DIR / "smoke" / "theme.mjs")], capture_output=True, text=True, timeout=60)
