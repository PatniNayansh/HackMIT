...
def test_the_figure_slide_shows_a_collapsed_machine_generated_description_and_the_others_show_none(tmp_path, sample):
    ...
    assert "Marks and labels only. Not scored." in out["slide8"]
...
def test_the_overview_ranks_shows_state_chips_and_labels_the_arc_propositions_covered(tmp_path, sample):
    ...
    # A coverage run is a count over a total, and must not borrow the rank chart's language.
    assert "ordinal" not in o and "four rungs" not in o
    assert "propositions the reader covered, over the number the claim has" in o
...
def test_the_neural_panel_names_itself_as_a_prediction_and_credits_the_model(sample, tmp_path):
    """The panel says what it is in its own heading and credit line, not in a warning. No wall of
    hedging -- but a reader still has to be able to tell, from the page alone, that these are
    numbers a model produced and which model produced them."""
    out = render(tmp_path, sample, "1")
    page = out["slide1"]
    assert "Predicted neural response" in page
    assert "TRIBE v2" in page
