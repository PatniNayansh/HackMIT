...
def test_upload_describes_only_the_figure_slides_once_and_the_run_hands_the_text_to_every_persona(tmp_path):
    ...
    import json
    recs = json.loads((tmp_path / "history" / up["run_id"] / "input.json").read_text(encoding="utf-8"))
    ...
    assert json.loads((tmp_path / "history" / again["run_id"] / "input.json").read_text(encoding="utf-8"))[1]["image_content"] == recs[1]["image_content"]
...
def test_with_no_helper_client_an_upload_still_works_and_the_figure_is_simply_not_described(tmp_path):
    ...
    import json
    recs = json.loads((tmp_path / "history" / up.json()["run_id"] / "input.json").read_text(encoding="utf-8"))
