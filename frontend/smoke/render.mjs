...
if (target.startsWith("http")) {
  ...
  route = (url) => {
    ...
    // The neural layer is precomputed, so a slide that was never run really does 404 -- null
    // here stands for that, and exercises the empty state rather than a fabricated brain.
    const neural = url.match(/slides\/(\d+)\/neural$/);
    if (neural) return (payload.neural ?? {})[neural[1]] ?? null;
    return payload.run;
  };
  globalThis.fetch = async (url) => {
    const body = route(url);
    if (body === null) return { ok: false, status: 404, statusText: "Not Found", json: async () => ({ detail: "no precomputed neural result" }) };
...
const arcSvg = ov.root.querySelectorAll("svg").find((v) => /across slide order|four rungs|propositions of the expert/i.test(v.attrs["aria-label"] ?? ""));
if (arcSvg) {
  out.arc_tooltips = [];
